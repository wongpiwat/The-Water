    ...
    public void saveItem(ActionEvent event){
        if (item == null) {
            if (!dissolvedOxygen.getText().isEmpty() && !celsius.getText().isEmpty() && !volumeWater.getText().isEmpty()) {
                ItemsDB.saveDB(DateUtilities.getDateNumber(),dissolvedOxygen.getText(), celsius.getText(), volumeWater.getText());
            ...
            //this.item.s
            ItemsDB.editDB(this.item.getNo(), DateUtilities.getDateNumber(),this.item.getDissolvedOxygen(),this.item.getCelsius(),this.item.getVolume());
            this.backToManagementWindow(event);
        }
    }
